...
- **Modular Routing Architecture**: Backend uses Express Router pattern with domain-based separation. Core domains extracted to dedicated router files: `notifications` (4 endpoints), `recordings` (3 endpoints), `amendments` (4 endpoints), `universities` (2 endpoints), `state-laws` (2 endpoints), `auth` (6 endpoints), and `profile` (9 endpoints). Shared cross-cutting logic resides in `server/routes/helpers/`. This modular structure reduced `routes.ts` from 3,051 to 1,906 lines (37% reduction) while improving maintainability, testability, and developer navigation. All routers are registered centrally via `app.use()` in `routes.ts`. Future work: Extract contracts router (~18 endpoints).
