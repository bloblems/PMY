...
- **Sharing & Referrals**: Features a Dropbox-style referral program using Resend API and allows sharing of consent documents via email. Accessible from the Profile page via the "Share" button, it includes three tabs: (1) Document Sharing - share contracts/recordings via email, (2) Invite Friends - send referral invitations with custom messages, display referral code/link, and QR code for easy sharing, (3) Referrals Dashboard - view referral stats (total, completed, pending) and referral history. Rate limiting is applied to prevent abuse.
