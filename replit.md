...
- **Sharing & Referrals**: Features a Dropbox-style referral program using Resend API and allows sharing of consent documents via email. Accessible from the Profile page via the "Share" button, it includes an Instagram-inspired QR code sharing screen with full-screen gradient backgrounds (3 PMY-branded black/green color schemes: PMY Green, Dark, Midnight), centered QR code card with PMY logo overlay, and three action buttons (Share, Copy link, Download). Also includes three tabs for additional functionality: (1) Document Sharing - share contracts/recordings via email, (2) Invite Friends - send referral invitations with custom messages, display referral code/link, (3) Referrals Dashboard - view referral stats (total, completed, pending) and referral history. Rate limiting is applied to prevent abuse.
- **Instagram-Style Profile**: An Instagram-inspired profile page at `/profile` displays user avatar (with fallback to initials), name from Supabase metadata, stats row (total contracts, active contracts, total recordings), bio, website URL, and tab navigation to switch between contracts and recordings grid views. Profile fields (profilePictureUrl, bio, websiteUrl) are stored in the `users` table and editable via `/api/profile` PATCH endpoint.
- **User Preferences**: A dedicated preferences page at `/settings/preferences` allows users to set default values for consent flows to improve efficiency. Preferences include: default university selection, state of residence, default encounter type (casual, romantic, committed), and default contract duration (1 hour, 4 hours, 1 day, 3 days, 1 week). All preference fields are optional. These defaults prepopulate new consent flows but do not override in-progress flows resumed from storage. Managed via GET/PATCH `/api/profile/preferences` endpoints. The ConsentFlowContext loads preferences on mount and applies them when creating new flows.

### System Design Choices
- **Database Schema**: Includes tables for `universities`, `consentRecordings`, `consentContracts`, `universityReports`, `users`, `verificationPayments`, and `payment_methods`. The `consentContracts` table includes optional duration fields: `contract_start_time` (timestamp), `contract_duration` (integer), and `contract_end_time` (timestamp). The `user_profiles` table contains `savedSignature`, `dataRetentionPolicy`, `stripeCustomerId`, `referralCode`, and user preference fields (`defaultUniversityId`, `stateOfResidence`, `defaultEncounterType`, `defaultContractDuration`). `payment_methods` stores metadata, while Stripe manages sensitive payment data.
...
- **Navigation**: A bottom navigation bar with "Create", "Tools", "Contracts", and "Profile" tabs. The "Create" tab serves as the primary entry for the consent flow. The "Profile" tab displays an Instagram-style profile page with user stats (total contracts, active contracts, recordings), bio, website URL, and grid views of contracts/recordings. Sharing functionality (document sharing, referral invitations, QR codes) is accessible via the "Share" button within the Profile page, which navigates to `/share`.
