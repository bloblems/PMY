...
- **Navigation**: A bottom navigation bar with "Create", "Tools", "Contracts", and "Profile" tabs. The Contracts tab displays Active Contracts (contracts still within their agreed duration period) and Completed Contracts (contracts that have fully transpired their duration) in separate sections. When collaborative contracts feature is enabled, it also includes sub-navigation tabs: Drafts (pending approval) and Inbox (received invitations).
